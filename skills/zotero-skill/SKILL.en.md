...
**MCP not available?** Check: is port 23120 up? (`curl -s http://127.0.0.1:23120/ping` → `pong`). If not, fall back to the Local HTTP API section below.
...
`find_item_by_identifier(doi?, isbn?)` — Find item by DOI or ISBN. Returns `key`, `title`, `itemType`, `date`, `creators`. Use the returned `key` with `get_item_by_key` for full details.
...
| `search_annotations(q?)` | Search annotations across the entire library | `q`, `type`, `itemKey`, `detailed` |
...
For `search_annotations`: use `detailed: true` for full content; the default is a preview (truncated).
...
**Bulk ingest (all items since a date):**
...
Use only when MCP is unavailable. All requests require the header `Zotero-Allowed-Request: true` and `userID = 0`.
...
req = urllib.request.Request(
    'http://localhost:23119/api/users/0/items?q=SEARCHTERM&limit=20&format=json',
...
Write operations always require `ZOTERO_API_KEY` and `ZOTERO_USER_ID`. There is no MCP alternative — the MCP server is read-only.

If credentials are not set, ask the user to:
1. Go to https://www.zotero.org/settings/keys
2. Note the **User ID** (top right)
3. Create an API key with library read/write access
...
Same pattern as above: GET item → read `version` → PATCH or DELETE with the `If-Unmodified-Since-Version` header.
...
- **MCP write operations**: The MCP server is read-only. All writes go through the Web API or Connector API.
