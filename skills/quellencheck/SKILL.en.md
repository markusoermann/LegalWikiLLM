...
description: >
  ...
  Use this skill whenever the user asks to: check, verify, or validate references/sources/bibliography
  in a thesis, master's thesis, bachelor's thesis, dissertation, research paper, or any document with a bibliography.
  Also trigger when the user says things like "are the sources real?", "do the sources check out?",
  "check the bibliography", "check if sources are real", "verify my references".
...
- Returns: status (OK / ERROR), CrossRef title, CrossRef year, HTTP code
...
- 403: Access denied (page exists but requires login — mark as UNCLEAR, not ERROR)
- 404: Not found — ERROR
- 5xx / timeout / connection error: ERROR
- 523 (Cloudflare): Server unreachable — ERROR
...
If no match is found in any database: mark as "unverifiable".
...
```
## Source Check: [Document title] (N entries)
...
### MISSING INFORMATION (unverifiable)
...
### ALL REMAINING SOURCES (approx. N)
OK — DOIs resolved ✓ · URLs reachable ✓ · details plausible ✓
```
...
If it fails again, mark the entry as "not checked (API limit)" and continue.
